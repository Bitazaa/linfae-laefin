...
var App={
  ...
  customer:{
    ...
    _customerNoteLabel:function(){
      var cat=String(App.state._deliveryCategoryType||'village');
      return cat==='village'?'หมายเหตุ / Comment':'หมายเหตุเพิ่มเติม / Comment';
    },
    applyOrderInfoLabels:function(){
      ...
      if(customerNoteInput)customerNoteInput.placeholder=(isVillage?'เช่น ไม่เผ็ด, ขอช้อนเพิ่ม, โทรก่อนส่ง':'รายละเอียดเพิ่มเติม / Comment');
    },
      ...
      App.ui.confirm('ยืนยันยกเลิกคำสั่งซื้อ?',function(ok){
        ...
        App.api.silent('cancelTempOrder',[App.state.orderId],function(res){
          if(res&&res.success===false){
            ...
            App.ui.toast((res&&res.message)||'ยกเลิกออเดอร์ไม่สำเร็จ','warn');
  ...
  admin:{
    ...
    ensureCanEdit:function(){
      ...
      App.ui.toast('บัญชี '+role+' ดูข้อมูลได้อย่างเดียว','warn');
      return false;
    },
    ...
    clearAllOrders:function(){
      ...
      App.ui.confirm('ยืนยันล้างข้อมูลรายการลูกค้าทั้งหมด?',function(ok){
        ...
        App.api.call('clearAllOrders',[App.state.adminToken],function(res){
          if(App.admin._auth(res))return;
          if(!res||!res.success){App.ui.toast((res&&res.message)||'ไม่สามารถล้างข้อมูลได้','warn');return;}
    ...
    _auth(res){if(res&&!res.success&&(res.code==='NO_SESSION'||res.code==='EXPIRED')){App.ui.toast('Session หมดอายุ','warn');setTimeout(function(){App.admin.logout();},1500);return true;}return false;},
    ...
    markSelectedPrinted:function(type){
      ...
      App.api.call('markOrdersPrinted',[ids,type,App.state.adminToken],function(res){
        ...
        App.ui.toast('อัปเดตสถานะพิมพ์แล้ว','success');
    ...
    openTopicEdit(idx){
      ...
      var topicStatus=(t&&String(t.status||'').toLowerCase()==='inactive')?'inactive':'active';
      setVal('tf-id',t?t.id:'');setVal('tf-name',t?t.group_name:'');setVal('tf-required',t?String(t.is_required||'false'):'false');setVal('tf-status',topicStatus);setVal('tf-type',t?t.type:'single');setVal('tf-stock',topicStock);
    ...
    saveShopAvailability:function(opts){
      ...
      if(App.admin.isReadOnly()){
        if(!opts.silent)App.ui.toast('บัญชี '+String(App.state.adminRole||'guest')+' ดูข้อมูลได้อย่างเดียว','warn');
        return;
      }
      var getVal=function(id){var el=document.getElementById(id);return el?el.value:'';};
      var openStart=getVal('s-open-start'),openEnd=getVal('s-open-end');
    ...
    _refreshOrdersAfterMutation:function(opts){
      ...
      if(opts.manual){
        setTimeout(function(){App.ui.toast('รีเฟรชข้อมูลล่าสุดแล้ว','success');},350);
      }
    },
    ...
    _silentRefreshOrders:function(opts){
      ...
      App.api.call('getOrders',[reqFilters,App.state.adminToken],function(res){
          ...
          if(newFp!==App.admin._ordersFp){
            ...
            if(hasNew){
              App.ui.toast('มีออเดอร์ใหม่เข้ามา!','success');
              App.admin._playNewOrderSound();
            }
            if(opts.manual)App.ui.toast('รีเฟรชข้อมูลล่าสุดแล้ว','success');
          }else if(opts.manual){
    ...
    _countNewOrders:function(){
      return (App.admin._ordersData||[]).filter(function(o){
        return String(o&&o.status||'').trim().toLowerCase()==='paid';
      }).length;
    },
    ...
    acceptAllNewOrders:function(){
      ...
      var targets=rows.filter(function(o){return String(o.status||'').trim().toLowerCase()==='paid';});
      if(!targets.length){App.ui.toast('ไม่มีออเดอร์ใหม่ให้รับ','warn');return;}
      var btn=document.getElementById('accept-all-new-btn');
    ...
    exportOrdersExcel:function(){
      ...
      if(!rows.length){App.ui.toast('ไม่มีข้อมูลสำหรับ Export','warn');return;}
    ...
    confirmExportPdf:function(){
      ...
      if(!base.length){App.ui.toast('ไม่มีข้อมูลสำหรับ Export','warn');return;}
      var selected=[];
